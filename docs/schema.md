...
- **Dinero — siempre `numeric`, nunca `float`/`double`, escala unificada (corregido 2026-07-11).** Las 21 columnas monetarias del modelo usan `numeric(18,0)` (Guaraníes, sin centavos) — hasta esa fecha había una inconsistencia real de escala (15 columnas en `numeric(18,2)` contra 6 en `numeric(18,0)`), corregida unificando todo a `numeric(18,0)` tras confirmar contra datos reales que ninguna de las 15 columnas tenía valores no enteros (0 pérdida de datos). Porcentajes son `numeric(5,2)` (2 declarados vía `HasColumnType`, 7 vía `HasPrecision(5,2)`); hay un caso suelto `numeric(14,2)` en `Tratamiento`, sin tocar.
...
- **Enums de negocio se guardan como `int`** (31 configuraciones con `HasConversion<int>()` — incluye los 6 campos migrados de string libre el 2026-07-11, ver el patrón transversal más abajo). No hay `CHECK` en la base que valide el rango — un valor fuera de rango entra sin error por SQL crudo. Tampoco hay `CHECK` para montos `>= 0` ni stock no negativo.
...
- **Estados/tipos como enum, sin excepciones (corregido 2026-07-11)** — hasta esa fecha había una inconsistencia real entre módulos: los más nuevos (`Venta`, `TrabajoPedido`, `PedidoProveedor`, `Egreso`) usaban enum C# con `HasConversion<int>()`; los más viejos (`MovimientoStock.Tipo`/`.Estado`, `ConteoInventario.Estado`, `TransferenciaStock.Estado`, más `MotivoMovimiento.Tipo` y `NotificacionInterna.Tipo`) usaban `string` libre. Los 6 campos se migraron a enum C# (migración `061_EnumsEstadoLibre`) sin cambiar el vocabulario expuesto por la API — mismos nombres de siempre, ahora tipados — salvo `NotificacionInterna.Tipo`, que pasó de snake_case (`bajo_stock`) a PascalCase (`BajoStock`) para seguir la convención de C#, con el único ajuste de frontend correspondiente (`NotificacionesView.vue`).
...
- **Scoping por sucursal — aislado centralmente vía EF Global Query Filter (agregado 2026-07-11)**, no manual por servicio. La mayoría de entidades transaccionales de los Grupos B y C tienen `SucursalId` (nullable solo donde la entidad es intencionalmente global); las 14 con dimensión de sucursal quedan restringidas automáticamente en toda query (list, GetById, Include) a la sucursal del usuario actual, sin depender de que cada servicio nuevo recuerde aplicar el guard a mano. Ver `architecture.md` § Multi-sucursal.
